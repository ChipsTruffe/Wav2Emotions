...
from config import MELD_FOLDER, MODEL_CONFIG
...
def parse_args():
    parser = argparse.ArgumentParser(description="Run MELD speech2text2emotion with Whisper + DistilBERT.")
    parser.add_argument("--data-folder", default=MELD_FOLDER)
    parser.add_argument("--meld-split", default="train")
    parser.add_argument("--output-root", default="results/meld_speech2text2emotion")
...
def build_texts(metadata, config, cache_dir, device):
    ...
    cache_tag = build_cache_tag(metadata["audio_path"].tolist(), config.whisper_model_name, "meld_whisper")
    cache_path = cache_dir / f"meld_whisper_texts_{cache_tag}.npz"
...
def build_text_features(texts, config, cache_dir, device):
    cache_tag = build_cache_tag(texts, config.text_model_name, "meld_s2t_text")
    cache_path = cache_dir / f"meld_s2t_text_features_{cache_tag}.npz"
...
def main():
    ...
    output_dir = build_run_dir(config.output_root, "meld", config.run_name)
    cache_dir = build_cache_dir(config.cache_root, "meld_speech2text2emotion")

    print(f"Data folder: {config.data_folder}")
    print(f"Split: {config.meld_split}")
    print(f"Output directory: {output_dir}")
    print(f"Cache directory: {cache_dir}")
    print(f"Device: {device}")

    metadata = load_meld_metadata(config.data_folder, split=config.meld_split, max_samples=config.max_samples)
    metadata.to_csv(output_dir / "metadata.csv", index=False)
    print(f"Loaded {len(metadata)} MELD samples")

    print("Transcribing MELD audio with Whisper...")
    texts = build_texts(metadata, config, cache_dir, device)
    metadata = metadata.copy()
    metadata["whisper_text"] = texts
    metadata.to_csv(output_dir / "metadata_with_whisper.csv", index=False)

    print("Embedding transcriptions with DistilBERT...")
    X = build_text_features(texts, config, cache_dir, device)
    ...
    run_config["pipeline"] = "meld_speech2text2emotion"
    ...
    metrics = save_training_artifacts(
        ...
        title_prefix="MELD Speech2Text2Emotion",
    )
